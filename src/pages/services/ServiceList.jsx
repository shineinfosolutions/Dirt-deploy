...
const ServiceList = () => {
  ...
  const fetchServices = async (pageNumber = 1) => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/service/pagination?page=${pageNumber}&limit=${limit}`);
  ...
  const searchServices = async () => {
    ...
    setIsSearching(true);
    try {
      const res = await axios.get(`http://localhost:5000/service/search?q=${searchQuery}`);
      setServices(res.data.data || []);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/service/delete/${id}`);
