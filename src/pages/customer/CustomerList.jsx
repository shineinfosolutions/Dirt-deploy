...
const CustomerList = () => {
  ...
  const fetchCustomers = async (pageNumber = 1) => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/custdirt/pagination?page=${pageNumber}&limit=${limit}`);
  ...
  const searchCustomers = async () => {
    ...
    setIsSearching(true);
    try {
      const res = await axios.get(`http://localhost:5000/custdirt/search?q=${searchQuery}`);
      setCustomers(res.data.data || []);
      setLoading(false);
    } catch (err) {
      setCustomers([]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/custdirt/delete/${id}`);
